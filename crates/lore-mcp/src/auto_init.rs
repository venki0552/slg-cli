...
pub fn initializing_response(tool_name: &str) -> Value {
    json!({
        "status": "initializing",
        "message": "lore is indexing this repository for the first time. Estimated time: ~15 seconds. Run your query again shortly.",
        "eta_seconds": 15,
        "tool": tool_name
    })
}
...
mod tests {
    ...
    fn test_initializing_response() {
        let resp = initializing_response("lore_why");
        assert_eq!(resp["status"], "initializing");
        assert_eq!(resp["tool"], "lore_why");
