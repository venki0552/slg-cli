...
pub async fn index_full_branch(
    ...
    let reference = repo
        ...
        .map_err(|e| LoreError::Git(format!("Branch '{}' not found: {}", branch, e)))?;

    let oid = reference
        .target()
        .ok_or_else(|| LoreError::Git(format!("Branch '{}' has no target", branch)))?;
...
fn build_diff_info(
    ...
    .map_err(|e| LoreError::Git(format!("Diff iteration failed: {}", e)))?;
