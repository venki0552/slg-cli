...
/// Build the lore block for a hook.
fn build_hook_block(content: &str) -> String {
    format!("{}\n{}\n{}", HOOK_HEADER, content, HOOK_FOOTER)
}

/// Install git hooks for lore. Returns list of hooks installed/updated.
    ...
    for (name, content) in hook_definitions() {
        ...
        if hook_path.exists() {
            ...
            if existing.contains(HOOK_HEADER) {
                // Update existing lore block
                let updated = replace_lore_block(&existing, &block);
                std::fs::write(&hook_path, updated).map_err(|e| {
                    LoreError::Git(format!("Failed to update hook {}: {}", name, e))
                })?;
                debug!("Updated hook: {}", name);
            } else {
                // Append lore block to existing hook
                let appended = format!("{}\n\n{}\n", existing.trim_end(), block);
                std::fs::write(&hook_path, appended).map_err(|e| {
                    LoreError::Git(format!("Failed to append to hook {}: {}", name, e))
                })?;
...
/// Remove lore blocks from all hook files.
pub fn remove_hooks(repo_path: &Path) -> Result<(), LoreError> {
    let hooks_dir = repo_path.join(".git").join("hooks");
    if !hooks_dir.exists() {
        return Ok(());
    }
    ...
    for (name, _) in hook_definitions() {
        ...
        let cleaned = remove_lore_block(&existing);
...
/// Check if all 4 hooks have the lore block installed.
...
/// Replace the lore block within existing content.
fn replace_lore_block(content: &str, new_block: &str) -> String {
...
/// Remove the lore block from content.
fn remove_lore_block(content: &str) -> String {
    if let (Some(start), Some(end)) = (content.find(HOOK_HEADER), content.find(HOOK_FOOTER)) {
        let before = &content[..start];
        let after = &content[end + HOOK_FOOTER.len()..];
