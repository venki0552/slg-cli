...
pub fn get_delta_commits(
    ...
    let base_ref = repo
        ...
        .map_err(|e| LoreError::Git(format!("Base branch '{}' not found: {}", base_branch, e)))?;
    ...
    let base_oid = base_ref
        .target()
        .ok_or_else(|| LoreError::Git("Base branch has no target".to_string()))?;
    let feature_oid = feature_ref
        .target()
        .ok_or_else(|| LoreError::Git("Feature branch has no target".to_string()))?;

    let merge_base = repo
        .merge_base(base_oid, feature_oid)
        .map_err(|e| LoreError::Git(format!("Failed to find merge base: {}", e)))?;
