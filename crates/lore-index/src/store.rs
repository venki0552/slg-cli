...
use lore_core::types::{CommitDoc, CommitIntent, IndexMetadata};
...
impl IndexStore {
    ...
    pub fn open(path: &Path) -> Result<Self, LoreError> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                LoreError::Database(format!("Failed to create index directory: {}", e))
            })?;
        }
    ...
    pub fn create_schema(&self) -> Result<(), LoreError> {
        self.conn
            .execute_batch(
                "
            ...
            .map_err(|e| LoreError::Database(format!("Failed to create schema: {}", e)))?;
    ...
    pub fn store_commit(
        ...
        .map_err(|e| LoreError::Database(format!("Failed to insert commit: {}", e)))?;
    ...
    pub fn commit_exists(&self, hash: &str) -> Result<bool, LoreError> {
        let count: i64 = self
            .conn
            .query_row(
            ...
            .map_err(|e| LoreError::Database(format!("Query failed: {}", e)))?;
        Ok(count > 0)
    }

    /// Get a commit document by hash.
    pub fn get_commit(&self, hash: &str) -> Result<Option<CommitDoc>, LoreError> {
        let result = self
            .conn
            .query_row(
    ...
    pub fn list_all_hashes(&self) -> Result<Vec<String>, LoreError> {
        let mut stmt = self
            .conn
            .prepare("SELECT hash FROM commits ORDER BY timestamp DESC")
            .map_err(|e| LoreError::Database(format!("Prepare failed: {}", e)))?;
    ...
    pub fn get_embedding(&self, hash: &str) -> Result<Option<Vec<f32>>, LoreError> {
        let result = self
            .conn
            .query_row(
    ...
    pub fn vector_search(
        ...
        query_embedding: &[f32],
        top_k: usize,
    ) -> Result<Vec<(String, f32)>, LoreError> {
        let mut stmt = self
            .conn
    ...
    pub fn update_last_accessed(&self) -> Result<(), LoreError> {
        let now = chrono::Utc::now().timestamp();
        self.conn
            .execute(
            ...
            .map_err(|e| LoreError::Database(format!("Update failed: {}", e)))?;
        Ok(())
    }
    ...
    pub fn get_metadata(
        ...
        branch: &str,
        base_branch: &str,
    ) -> Result<IndexMetadata, LoreError> {
