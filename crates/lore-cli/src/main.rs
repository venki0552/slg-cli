...
async fn run_health() -> Result<(), LoreError> {
    let cwd = std::env::current_dir().map_err(LoreError::Io)?;

    let (git_root, branch, indexed, size_kb) = match lore_git::detector::find_git_root(&cwd) {
        Ok(root) => {
            let branch = lore_git::detector::get_current_branch(&root)
...
/// Print stable repo hash.
fn run_repo_hash() -> Result<(), LoreError> {
    let cwd = std::env::current_dir().map_err(LoreError::Io)?;
    let git_root = lore_git::detector::find_git_root(&cwd)?;
    let hash = lore_git::detector::compute_repo_hash(&git_root);
    println!("{}", hash);
    Ok(())
}

/// Index a single commit by hash (used by post-commit hook).
async fn run_index_commit(hash: &str) -> Result<(), LoreError> {
    let cwd = std::env::current_dir().map_err(LoreError::Io)?;
    let git_root = lore_git::detector::find_git_root(&cwd)?;
    ...
    let store = lore_index::store::IndexStore::open(&index_path)?;
    ...
    let embedder = lore_index::embedder::Embedder::new()?;
...
/// Print index path for current repo.
fn run_index_path() -> Result<(), LoreError> {
    let cwd = std::env::current_dir().map_err(LoreError::Io)?;
    let git_root = lore_git::detector::find_git_root(&cwd)?;
    let repo_hash = lore_git::detector::compute_repo_hash(&git_root);
    let branch =
