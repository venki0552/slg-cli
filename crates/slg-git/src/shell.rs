...
const SHELL_BLOCK_HEADER: &str = "# slg — semantic git intelligence — DO NOT EDIT THIS BLOCK";
const SHELL_BLOCK_FOOTER: &str = "# end slg";
...
pub fn install_shell_integration(shell: &Shell) -> Result<bool, SlgError> {
    let rc_path = shell_rc_path(shell)
        .ok_or_else(|| SlgError::Config("Could not determine shell RC file path".to_string()))?;
    ...
    std::fs::write(&rc_path, new_content)
        .map_err(|e| SlgError::Git(format!("Failed to write shell RC: {}", e)))?;
...
_slg_chpwd() {{
  if [ -d ".git" ]; then
    local _slg_hash
    _slg_hash=$(slg _repo-hash 2>/dev/null)
    if [ -n "$_slg_hash" ] && [ ! -f "$HOME/.slg/indices/$_slg_hash/main.db" ]; then
      slg index --background --silent 2>/dev/null &
    fi
  fi
}}
autoload -U add-zsh-hook
add-zsh-hook chpwd _slg_chpwd
{footer}"#,
...
PROMPT_COMMAND="_slg_chpwd; $PROMPT_COMMAND"
{footer}"#,
            header = SHELL_BLOCK_HEADER,
            footer = SHELL_BLOCK_FOOTER,
        ),
        Shell::Fish => format!(
            r#"{header}
function _slg_chpwd --on-variable PWD
  if test -d ".git"
    set -l _slg_hash (slg _repo-hash 2>/dev/null)
    if test -n "$_slg_hash"; and not test -f "$HOME/.slg/indices/$_slg_hash/main.db"
      slg index --background --silent 2>/dev/null &
    end
  end
end
{footer}"#,
        ...
        Shell::PowerShell | Shell::Unknown => format!(
            r#"{header}
# slg auto-index on directory change (PowerShell)
# Not yet supported — run slg index manually
{footer}"#,
            header = SHELL_BLOCK_HEADER,
            footer = SHELL_BLOCK_FOOTER,
        ),
