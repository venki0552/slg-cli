...
pub async fn run_mcp_server() -> Result<(), SlgError> {
    ...
    info!("slg MCP server started");

    let mut line = String::new();
    loop {
        line.clear();
        let bytes_read = reader.read_line(&mut line).await.map_err(SlgError::Io)?;
...
async fn handle_method(method: &str, request: &Value) -> Result<Value, SlgError> {
    match method {
        "initialize" => Ok(json!({
        ...
        _ => Err(SlgError::Config(format!("Unknown method: {}", method))),
    }
}

async fn handle_tool_call(tool_name: &str, params: &Value) -> Result<Value, SlgError> {
    ...
    let git_root = match detector::find_git_root(&cwd) {
        ...
        Err(_) => {
            return Ok(json!({
                "content": [{"type": "text", "text": "Error: not in a git repository. Run `slg init` first."}],
                "isError": true
            }));
        }
    };
...
mod tests {
    ...
    async fn test_handle_initialize() {
        ...
        assert_eq!(result["serverInfo"]["name"], "slg");
    }
