...
impl OutputGuard {
    ...
    pub fn check_and_sanitize(&self, output: &str, max_bytes: usize) -> String {
        ...
        if result.contains("</slg_retrieval>") {
            let count = result.matches("</slg_retrieval>").count();
            if count > 1 {
                self.log_security_event(
                    "xml_structure_risk",
                    "Multiple </slg_retrieval> tags found",
                );
            }
        }
    ...
    fn log_security_event(&self, event_type: &str, detail: &str) {
            ...
            .open(&log_path)
        {
            ...
            // Intentionally ignore write errors — logging should never crash slg
