...
/// SECURITY: Build a safe index path that cannot escape ~/.slg/indices/<repo_hash>/.
/// This is the ONLY way to get an index path — no other code constructs paths.
pub fn safe_index_path(repo_hash: &str, branch_name: &str) -> Result<PathBuf, SlgError> {
    ...
    if !candidate_str.starts_with(&*base_str) {
        return Err(SlgError::PathTraversal(format!(
    ...
    for component in candidate.components() {
        if let std::path::Component::ParentDir = component {
            return Err(SlgError::PathTraversal(
...
/// Returns ~/.slg/ — creates with restricted permissions if not exists.
pub fn slg_home() -> PathBuf {
...
/// Returns ~/.slg/models/ — creates if not exists.
pub fn models_dir() -> PathBuf {
    let dir = slg_home().join("models");
    if !dir.exists() {
...
/// Returns ~/.slg/indices/<repo_hash>/ — creates with restricted permissions if not exists.
pub fn indices_base(repo_hash: &str) -> PathBuf {
    // SECURITY: repo_hash is SHA256 hex, only safe chars
    let dir = slg_home().join("indices").join(repo_hash);
    if !dir.exists() {
        let _ = std::fs::create_dir_all(&dir);
...
/// Returns ~/.slg/security.log
pub fn security_log_path() -> PathBuf {
    slg_home().join("security.log")
}
...
mod tests {
    ...
    #[test]
    fn test_slg_home_path() {
        let home = slg_home();
        assert!(home.to_string_lossy().contains(".slg"));
    }
