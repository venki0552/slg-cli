...
impl Embedder {
    ...
    pub fn new() -> Result<Self, SlgError> {
        let cache_dir = slg_security::paths::models_dir();
        ...
        let model = TextEmbedding::try_new(options)
            .map_err(|e| SlgError::Embedding(format!("Failed to load embedding model: {}", e)))?;
    ...
    pub fn embed_commit(&self, doc: &CommitDoc) -> Result<Vec<f32>, SlgError> {
        let text = build_commit_text(doc);
        self.embed_text(&text)
    }

    /// Embed a raw query string.
    pub fn embed_query(&self, query: &str) -> Result<Vec<f32>, SlgError> {
    ...
    pub fn embed_batch(&self, docs: &[&CommitDoc]) -> Result<Vec<Vec<f32>>, SlgError> {
        let texts: Vec<String> = docs.iter().map(|d| build_commit_text(d)).collect();
        let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();

        self.model
            .embed(text_refs, None)
            .map_err(|e| SlgError::Embedding(format!("Batch embedding failed: {}", e)))
    }

    /// Embed a single text string.
    fn embed_text(&self, text: &str) -> Result<Vec<f32>, SlgError> {
        let results = self
            .model
            .embed(vec![text], None)
            .map_err(|e| SlgError::Embedding(format!("Embedding failed: {}", e)))?;

        results
            .into_iter()
            .next()
            .ok_or_else(|| SlgError::Embedding("No embedding result returned".to_string()))
    }
