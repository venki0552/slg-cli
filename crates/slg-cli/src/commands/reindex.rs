...
pub async fn run(args: ReindexArgs) -> Result<(), SlgError> {
    ...
    let sanitizer = slg_security::sanitizer::CommitSanitizer;
    let redactor = slg_security::redactor::SecretRedactor::new();
    ...
    for hash in &new_commits {
        ...
        let raw_doc = slg_git::ingestion::build_raw_commit_doc(&repo, &commit, &branch)?;
